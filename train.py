...
import time
from torch.optim.lr_scheduler import ReduceLROnPlateau #torch.optim.lr_scheduler.ReduceLROnPlateau as ReduceLROnPlateau
...
def main() -> None:
    ...
    start = time.time()
    lr_decay_scheduler = ReduceLROnPlateau(optimizer, 'min', factor=0.5, patience=10, min_lr=0.0001)
    for epoch in range(begin_epoch, config.max_epoch):
        ...
        if config.param_search:
            ...
            lr_decay_scheduler.step(val_loss)
        ...
        if config.param_search:
            ...
            if cls_acc > best_acc:
                best_acc = cls_acc
                torch.save(model.state_dict(), os.path.join(result_path, 'best_val_acc.prm'))
        else:
            print(
